...
import com.acxdev.commonFunction.util.Toast.toasty
...
object IOtherView {
    ...
    fun TextView.showVersion(prefix: String) {
        try {
            text = "$prefix ${
                context.getCompatActivity()!!.packageManager.getPackageInfo(
                    context.getCompatActivity()!!.packageName, 0
                ).versionName
            }"
        } catch (e: PackageManager.NameNotFoundException) {
            context.getCompatActivity().toasty(Toast.WARNING, e.message.toString())
        }
    }
