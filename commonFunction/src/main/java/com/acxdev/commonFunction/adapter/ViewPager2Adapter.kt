...
class ViewPager2Adapter(fragmentActivity: FragmentActivity) :  FragmentStateAdapter(fragmentActivity) {
    ...
    fun sets(vararg fragments: Fragment) { for (i in listOf(*fragments).indices) this.listFragment.add(listOf(*fragments)[i]) }

    fun setWithTab(listFragment: MutableList<Pair<String,Fragment>>, tabLayout: TabLayout, viewPager2: ViewPager2) {
        this.listFragmentTabView = listFragment
        isSetupWithTabView = true
        TabLayoutMediator(tabLayout, viewPager2) { tab, pos -> tab.text = listFragment[pos].first }.attach()
    }
}